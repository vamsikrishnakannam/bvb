...
if(isset($_SESSION['id']) && isset($_SESSION['email']) && isset($_SESSION['name']))
{
	...
	//changes
	//cid needs to change for every question as by title
	$cid=3;
?>
...
<body>
	...
	{
		$code=$_POST['code'];
		$sql="select * from codedata where email='$email' and codeid='$cid';";
		$run=mysqli_query($conn,$sql);
		if(mysqli_num_rows($run)==0)
		{
			$s="INSERT INTO codedata VALUES ('','$email','$name','$cid','\r\n$code','0');";
		...
		{
			$s="UPDATE `codedata` SET `code`='\r\n$code' WHERE email='$email' AND codeid='$cid';";
			$r=mysqli_query($conn,$s);
			if($r)
			{
				//changes
				//sample input need to change
				$input=10;
				$result=request($code,$input);
			}
			else
	...
	if(isset($_POST['submit']))
	{
		$code=$_POST['code'];
		//changes
		//in and outputs need to change for test cases
		$in = array(25,1);
		$op= array(325,1);
		//end
		$output= array();
		...
		{
			$s="UPDATE `codedata` SET `code`='$code',`testcase`='$testcase' WHERE email='$email' AND codeid='$cid';";
			$r=mysqli_query($conn,$s);
		}
      ...
      <ul class="nav navbar-nav" style="float: right;">
		...
		<div class="col-lg-6" style="margin-top:3px; margin-right: 0px;">
				<div class="panel panel-default pb" style="margin: 0px;">
					...
					<div class="panel-body" style="height:465px;overflow-y: scroll;">
						<!-- changes-->
						<!-- Question need to change -->
						<h2>Sum of n Natural numbers</h2>
						<p>write a Java Program to count the sum of the n Natural Numbers</p>
						<p><b>Input Format :</b> Input contains a Integer value</p>
						<p><b>Input Constraints :</b>1<=Integer<=10^5</p>
						<p><b>Output Format :</b> Print the sum of digits</p>
					...
					<form action="" method="POST">
						<div class="form-group">
							<textarea id="d2" class="form-control pb" style="text-align:left; width: 100%; height: 400px; overflow-y: scroll;" spellcheck="false" name="code" required="" value="">
								<?php 
									$q="select * from codedata where email='$email' and codeid=$cid;";
		...
		<div class="panel panel-default pb">
			...
			<div class="panel-body" style="height:200px;overflow-y: scroll;">
				<p><b>Input:</b></p>
				<!-- changes sample input-->
				<p>10</p>
				<p><b>Excepted Output:</b></p>
				<!-- changes sample output-->
				<p>55</p>
				<p><b>Your output:</b></p>
